<?php
	...
	define('ERROR_LOG_FILE', 'logs/add_exp_app_errors.xml');
	
	include_once 'CustomException.php';
	
	session_start();
	
	if(!isset($_POST['categoty_of_exp'])){
	...
	try{
		...
		if ($connection_SQL->connect_errno!=0){ #brak połącznia z bazą danych
			...
			if(isset($_POST['categoty_of_exp'])){
				$zlote = $_POST['expense_amount_zl'];
				$grosze = $_POST['expense_amount_gr'];
				$date_form = $_POST['date_of_exp'];
				$date = date("Y-m-d", strtotime($date_form));
				$category = $_POST['categoty_of_exp'];
				...
				if($everything_all_right==FALSE){		
					header('Location: dodawanie-wydatku-lub-dochodu'); 
				 	...
				 	$result = $connection_SQL->query('SELECT MAX(id_expense) FROM expenses');
					if(!$result) throw new Exception($connection_SQL->error);
					$row = $result->fetch_assoc(); #tworzymy tablicę asocjasyjną
					$result->free(); #zwalniamy miejsce w rezultacie zapytania
					$last_id = $row['MAX(id_expense)'];
					
					if(!$connection_SQL->query("INSERT INTO expenses VALUES ($last_id+1, ".'"'.$date.'"'." , $category , $amount , ".'"'.$comment.'"'.")")) throw new Exception($connection_SQL->error);
					
					$user_id = $_SESSION['user_id'];
					
					if(!$connection_SQL->query("INSERT INTO users_expenses VALUES ($user_id ,$last_id+1 , 1 )")) throw new Exception($connection_SQL->error);
					
					$_SESSION['exp_succ']=TRUE;
					header('Location: dodawanie-wydatku-lub-dochodu'); 					
				}
			}
		}
